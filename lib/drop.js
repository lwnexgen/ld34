ig.module(
	...
	DropGame = ig.Game.extend({
		...
		this.score = this.screen.y % 8;
		
		// Do we need a new row?
		if( Math.abs((this.screen.y % 2)) <= offset ) {		    
		    console.log("Time for a new enemy! " + this.enemySpawned + " " + Math.abs((this.screen.y % 2)));
		...
		if( ig.input.pressed('bomb') ) {
		    this.spawnEntity( EntityBomb, this.player.pos.x, this.player.pos.y - 8 );
		}
	    },
		...
		this.font.draw( (Number(this.score).toFixed(2)).toString(), ig.system.width -2, 2, ig.Font.ALIGN.RIGHT );
	    }
	});
