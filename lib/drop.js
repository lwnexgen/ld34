ig.module(
	...
	EntityPlayer = ig.Entity.extend({
		...
		if (ig.input.pressed('swap')) {
		    if (this.pos.x == ig.game.LEFT_VINE) {
			this.pos.x = ig.game.RIGHT_VINE;
		    } else {
			this.pos.x = ig.game.LEFT_VINE;
		    }
		}
	...
	DropGame = ig.Game.extend({
		...
		for( var y = 0; y < 6; y++ ) {	
		    this.map[y] = (([this.row, this.row2]).slice())[Math.floor(Math.random() * 2)];
		}
		...
		console.log(grass);
		this.player = this.spawnEntity( EntityPlayer, this.LEFT_VINE, ig.system.height/2-2 );
		...
		if( Math.abs(this.screen.y % 8) <= offset ) {
			...
			this.spawnEntity(
			    EntityEnemy, 
			    ([this.LEFT_VINE, this.RIGHT_VINE])[Math.floor(Math.random() * 2)], 
			    this.screen.y 
			)
		...
		if( ig.input.pressed('bomb') ) {
		    this.spawnEntity( EntityBomb, this.player.pos.x, this.player.pos.y - (12) );
		}
